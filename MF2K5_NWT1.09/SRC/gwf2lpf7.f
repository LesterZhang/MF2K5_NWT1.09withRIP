...
#ifdef HDFDATA 
      IsHDF=.false.
	call HDF1_CheckifHDF(ILPFCB)
      if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(ILPFCB, KSTP, KPER,3)
      else
...
C4C-----RECORD CONTENTS OF BUFFER AND RETURN.
	...
	call HDF1_CheckifHDF(ILPFCB)
      if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(ILPFCB, KSTP, KPER,4)
      else
...
C5C-----RECORD CONTENTS OF BUFFER AND RETURN.
	...
	call HDF1_CheckifHDF(ILPFCB)
      if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(ILPFCB, KSTP, KPER,5)
      else
...
C9-----IF IBD FLAG IS SET RECORD THE CONTENTS OF THE BUFFER.
	...
	call HDF1_CheckifHDF(ILPFCB)
      if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(ILPFCB, KSTP, KPER,1)
      else    
...
C16-----IF SAVING CELL-BY-CELL FLOW IN 3-D ARRAY, WRITE THE ARRAY.
	...
	call HDF1_CheckifHDF(ILPFCB)
      if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(ILPFCB, KSTP, KPER,2)
      else    
      IF(IBD.EQ.1) CALL UBUDSV(KSTP,KPER,TEXT,
     1                   ILPFCB,BUFF,NCOL,NROW,NLAY,IOUT)
      endif
