...
!#ifdef HDFDATA
!      !initilize the file logical variable L_HDF
!      L_HDF=.false.
!#endif
C
...
C6------IF SO THEN CALL A ULASAV OR ULASV2 TO RECORD DRAWDOWN.
   ...
   79   CONTINUE
          ...
          !write the drawdown data to a HDF file    
         IF(maxval(IOFLG(1:NLAY,4)).NE.0)then
             IF(IFIRST.EQ.1) WRITE(IOUT,74) IDDNUN,KSTP,KPER
             call HDF1WriteHead_DD(IDDNUN, KSTP, KPER,2)
         endif
      endif
#endif        
C
...
C5------IF SO THEN CALL ULASAV OR ULASV2 TO SAVE HEAD.
   ...
   79   CONTINUE
         ...
         IF(maxval(IOFLG(1:NLAY,3)).NE.0)then
             IF(IFIRST.EQ.1) WRITE(IOUT,74) IHEDUN,KSTP,KPER
             !write the head data to a HDF file    
             call HDF1WriteHead_DD(IHEDUN, KSTP, KPER, 1)
         endif
       endif
#endif
C
...
C
#ifdef HDFDATA
C Check for HDF data format
      ELSE IF(LINE(ITYP1:ITYP2).EQ.'DATA(HDF)' .OR.
     1        LINE(ITYP1:ITYP2).EQ.'DATA(HDF)') THEN
         !FMTARG='HDF'
         !ACCARG='HDF'
         FMTARG='HDF'
         ACCARG='HDF'          

         !write(*,*) "HDF file detected"
         !write(*,*) FNAME(1:IFLEN), IU
#endif         
...
C13-----WRITE THE FILE NAME AND OPEN IT.
      ...
      if (FMTARG.ne.'HDF'.and.ACCARG.NE.'HDF')then
      !open file if they are not HDF file format     
...
50    FORMAT(1X,/1X,'OPENING ',A,/
     ...
     1      ACCESS=ACCARG,STATUS=FILSTAT,ACTION=FILACT,ERR=2000)
          ...
          !open file if they are HDF file by calling HDF file open subroutine
          WRITE(IOUT,50) FNAME(1:IFLEN),
     1     LINE(ITYP1:ITYP2),IU,FILSTAT,FMTARG,ACCARG

          
          fileCount=filecount+1
          hdf_file(fileCount)%File_UN=IU
          !L_HDF(filecount)=.true.

          !call HDF1AR()
      call HDF1PF(FNAME, IFLEN,filecount)
          
          !write(*,*) "need to add code to open the HDF file"
