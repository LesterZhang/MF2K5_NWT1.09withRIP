...
#ifdef HDFDATA      
      IsHDF=.false.
	call HDF1_CheckifHDF(IBCFDB)
	if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(IBCFCB, KSTP, KPER,1)
      else    
...
#ifdef HDFDATA      
      IsHDF=.false.
	call HDF1_CheckifHDF(IBCFCB)
	if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(IBCFCB, KSTP, KPER,2)
      else    
      IF(IBD.EQ.1) CALL UBUDSV(KSTP,KPER,TEXT,
     1                   IBCFCB,BUFF,NCOL,NROW,NLAY,IOUT)
      endif
...
C3C-----RECORD CONTENTS OF BUFFER AND RETURN.
	...
	call HDF1_CheckifHDF(IBCFCB)
	if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(IBCFCB, KSTP, KPER,3)
      else
...
C4C-----RECORD CONTENTS OF BUFFER AND RETURN.
	...
	call HDF1_CheckifHDF(IBCFCB)
      if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(IBCFCB, KSTP, KPER,4)
      else
...
C5C-----RECORD CONTENTS OF BUFFER AND RETURN.
	...
	call HDF1_CheckifHDF(IBCFCB)
      if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(IBCFCB, KSTP, KPER,5)
      else
