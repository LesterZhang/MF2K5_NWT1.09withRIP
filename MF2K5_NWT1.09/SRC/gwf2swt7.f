...
#ifdef HDFDATA  
      IsHDF=.false.
	call HDF1_CheckifHDF(ISWTCB)
111   if(IBD.EQ.1.and.IsHDF)then
          call HDF1WriteCBB(ISWTCB, KSTP, KPER,13)     
      else
      IF(IBD.EQ.1) CALL UBUDSV(KSTP,KPER,TEXT,ISWTCB,BUFF,NCOL,NROW,
     1                          NLAY,IOUT)
      endif
#else
