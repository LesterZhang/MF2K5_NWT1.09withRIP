...
#ifdef RIP_ET
! add support for RIP-ET support     
...
#ifdef HDFDATA
      if(filecount>=0)then
          !initialIZE the HDF varialble
          call HDF1AR()
          do i=1,filecount
              call HDF1WriteG(hdf_file(i)%fileID)
          ENDDO
      ENDIF    
#endif
...
#ifdef RIP_ET      
! Add support for RIP-ET
      IF(IUNIT(6).GT.0) CALL GWF2RIP3AR(IUNIT(6),IGRID) 
#endif      
...
#ifdef RIP_ET        
! add support for RIP-ET
        IF(IUNIT(6).GT.0) CALL GWF2RIP3RP(IUNIT(6),IGRID) 
#endif        
...
#ifdef RIP_ET
!Add support for RIP-ET 
            IF(IUNIT(6).GT.0) CALL GWF2RIP3FM(IGRID) 
#endif            
...
#ifdef RIP_ET
!Add support for RIP-ET
          IF(IUNIT(6).GT.0) CALL GWF2RIP3BD(KKSTP,KKPER,IGRID)
#endif          
...
#ifdef RIP_ET
!Add support for RIP-ET
      IF(IUNIT(6).GT.0) CALL GWF2RIP3DA(IGRID) 
#endif      
